...
BEGIN
    ...
    WHILE @@FETCH_STATUS = 0 BEGIN
        ...
        SET @CREATED_DT = DATEADD(DAY, ABS(CHECKSUM(NEWID()) % 3650), '2000-01-01');
        ...
        MERGE INTO [VIDEOS] AS [VIDEO]
            USING (VALUES (@USER_UID, @ACCESS_UID, @AGE_UID, @PATH, @TITLE, @DESCRIPTION, @CREATED_DT)) AS 
                [NEW_VIDEO] ([USER_UID], [ACCESS_UID], [AGE_RESTRICTIONS_UID], [PATH], [TITLE], [DESCRIPTION], [CREATED_DT])
            ON [VIDEO].[TITLE] = [NEW_VIDEO].[TITLE]
            WHEN NOT MATCHED THEN
                INSERT ([USER_UID], [ACCESS_UID], [AGE_RESTRICTIONS_UID], [CREATED_DT], [PATH], [TITLE], [DESCRIPTION]) VALUES
                    (@USER_UID, @ACCESS_UID, @AGE_UID, @CREATED_DT, @PATH, [NEW_VIDEO].[TITLE], [NEW_VIDEO].[DESCRIPTION]);
        FETCH NEXT FROM CUR INTO @USERNAME, @TITLE, @DESCRIPTION, @ACCESS, @AGE;
    END;
    CLOSE CUR;
    DEALLOCATE CUR;
