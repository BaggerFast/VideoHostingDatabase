...
IF NOT EXISTS (SELECT 1 FROM [INFORMATION_SCHEMA].[TABLE_CONSTRAINTS] WHERE [CONSTRAINT_TYPE] = 'PRIMARY KEY' AND [TABLE_NAME] = N'AGE_RESTRICTIONS') BEGIN
    ALTER TABLE [AGE_RESTRICTIONS] ADD CONSTRAINT [PK_AGE_RESTRICTIONS_UID] PRIMARY KEY CLUSTERED ([UID] ASC)
...
--DEFAULT
IF NOT EXISTS (SELECT 1 FROM [SYS].[DEFAULT_CONSTRAINTS] WHERE [NAME] = 'DF_AGE_RESTRICTIONS_UID') BEGIN
	ALTER TABLE [AGE_RESTRICTIONS] ADD CONSTRAINT [DF_AGE_RESTRICTIONS_UID] DEFAULT (NEWID()) FOR [UID];
	PRINT N'➕ DEFAULT KEY [DF_AGE_RESTRICTIONS_UID] WAS CREATED';
END;
