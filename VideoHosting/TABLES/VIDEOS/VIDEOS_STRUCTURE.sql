...
-- UNIQUE
IF NOT EXISTS (SELECT 1 FROM [INFORMATION_SCHEMA].[TABLE_CONSTRAINTS] WHERE [CONSTRAINT_NAME] = 'UQ_VIDEOS_TITLE_USER_UID' AND [TABLE_NAME] = N'VIDEOS') BEGIN
    ALTER TABLE [VIDEOS] ADD CONSTRAINT [UQ_VIDEOS_TITLE_USER_UID] UNIQUE([USER_UID], [TITLE])
    PRINT N'➕ UNIQUE KEY [UQ_VIDEOS_TITLE_USER_UID] WAS CREATED';
END;

-- DEFAULT
IF NOT EXISTS (SELECT 1 FROM [SYS].[DEFAULT_CONSTRAINTS] WHERE [NAME] = 'DF_VIDEOS_UID') BEGIN
	ALTER TABLE [VIDEOS] ADD CONSTRAINT [DF_VIDEOS_UID] DEFAULT (NEWID()) FOR [UID];
	PRINT N'➕ DEFAULT KEY [DF_VIDEOS_UID] WAS CREATED';
END;
