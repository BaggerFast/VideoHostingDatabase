...
END ELSE BEGIN
    ...
    BEGIN
        ...
        WHILE @@FETCH_STATUS = 0 BEGIN
            ...
            MERGE INTO [FK_VIDEO_TAGS] AS [VIDEO_TAG_FK]
            USING (VALUES (@TAG_UID , @VIDEO_UID)) AS [NEW_VIDEO_TAG_FK] ([TAG_UID], [VIDEO_UID])
                ON [VIDEO_TAG_FK].[TAG_UID] = @TAG_UID AND [VIDEO_TAG_FK].[VIDEO_UID] = @VIDEO_UID
                WHEN NOT MATCHED THEN
                    INSERT ([TAG_UID], [VIDEO_UID]) VALUES (@TAG_UID, @VIDEO_UID);
            FETCH NEXT FROM CUR INTO @TAG, @VIDEO;
        END;
        CLOSE CUR;
        DEALLOCATE CUR;
